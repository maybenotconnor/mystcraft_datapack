#scoreboard players enable @s archipeligotele
#scoreboard players enable @s cavetele
#scoreboard players enable @s icetele
scoreboard players enable @s overworldtele
scoreboard players enable @s sanctuarytele
scoreboard players enable @s smalltele
#scoreboard players enable @s twilighttele
#scoreboard players enable @s villagetele
#scoreboard players enable @s volcanotele
#scoreboard players enable @s rocktele
#scoreboard players enable @s warrentele
#scoreboard players enable @s skylandtele

scoreboard players set @s walking 0
scoreboard players set @s hastriggers 1

scoreboard players set @s lecternuse 0
scoreboard players set @s bookuse 0