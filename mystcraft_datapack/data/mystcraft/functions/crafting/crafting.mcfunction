...
#Player Book
execute as @e[type=item,nbt={Item:{id:"minecraft:player_head",Count:1b}},limit=1,sort=nearest,distance=0..1] at @s run summon item ~ ~0.1 ~ {PickupDelay:50,Tags:["fresh_playerbook"],Item:{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{display:{Name:'{"text":"Player Book","color":"red","italic":false}'},HideFlags:6,Unbreakable:1b,CustomModelData:689,playerbook:1b}}}
