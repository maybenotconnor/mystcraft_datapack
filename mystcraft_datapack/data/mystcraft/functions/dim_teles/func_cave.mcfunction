#drops dummy item
execute at @s[nbt={SelectedItem:{tag:{cavebook:1b}}}] run summon minecraft:item ~ ~0.1 ~ {Age:-32750,Tags:["droppedbook"],PickupDelay:50,Item:{id:"minecraft:written_book",Count:1b,tag:{display:{Name:'{"text":"WorldBook","color":"Black","bold":true,"italic":false,"underlined":false,"strikethrough":false,"obfuscated":false}'},HideFlags:7,CustomModelData:691}}}
...
#Removes Book on Use
replaceitem entity @s[nbt={SelectedItem:{tag:{cavebook:1b}}}] weapon air
...
#run tp
execute as @s at @s in mystworlds:cavesworld run tp @s ~ ~ ~
